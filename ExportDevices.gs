...
function exportCBs()
{
  ...
  var deviceArray = [["Serial Number", "Org Unit Path", "Location", "AssetID", "User", "Notes", "OS Version", "Most Recent User", "Last Sync", "Status", "MAC", "AUE", "Model"]];
  ...
  do
  {
    ...
    if (devices && devices.length > 0) {
      for (i = 0; i < devices.length; i++) {
        ...
        deviceArray.push([device.serialNumber, device.orgUnitPath, location, asset, user, note, osversion, recentUser, lastsync, status, macAddress, autoUpdateExpiration, model])
      }
    }
